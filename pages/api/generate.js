...
export default async function AI(req, res) {
  async function completion(new_message_text, settings_text, past_messages) {
    ...
    try {
      ...
      if (e instanceof openai.error.APIError) {
        // APIエラーを処理するコード
        await res.status(200).json({
          result: e,
        });
      } else if (e instanceof openai.error.APIConnectionError) {
        // 接続エラーを処理するコード
        await res.status(200).json({
          result: e,
        });
      } else if (e instanceof openai.error.RateLimitError) {
        // レート制限エラーを処理するコード
        await res.status(200).json({
          result: e,
        });
      } else {
        // その他のエラーを処理するためのカスタムコードを実行する
        await res.status(200).json({
          result: e,
        });
      }
